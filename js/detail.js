        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 space-y-6">
            ...
            <div class="bg-slate-950/60 p-5 rounded-xl border border-slate-800/80">
                  <a href="https://gdladder.com/level/${level.id}" 
                     class="text-sm font-bold text-slate-300 tracking-wider underline hover:text-violet-300"> 
                     GDDL link
                  </a>
                  <h3 class="text-sm font-bold text-slate-400 tracking-wider mb-2 mt-2">REVIEW / 心得</h3>
                  <p class="text-slate-200 leading-relaxed whitespace-pre-line text-base">${level.review ? `${level.review}` : '（尚未更新）'}</p>
            </div>
        </div>
    `;
}
